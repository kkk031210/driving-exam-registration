...
void appointment_menu(void) {
    ...
    do {
        ...
        choice = get_choice();
        switch (choice) {
            case 1: book_appointment(); break;      /* 预约考试 */
            case 2: list_appointments(); break;    /* 查看预约列表 */
            case 3: query_appointment(); break;    /* 精确查询 */
            case 4: cancel_appointment(); break;   /* 取消预约 */
            case 5: update_appointment(); break; /* 修改预约 */
            case 0: break;                         /* 直接返回 */
            default: printf("无效选择！\n"); pause_screen();
        }
    } while (choice != 0);
...
void book_appointment(void) {
    ...
    for (int i = 0; i < student_count; i++) {
        if (students[i].id == student_id) { found = 1; break; }
    }
    if (!found) {
        printf("[错误] 考生ID不存在！\n");
        pause_screen();
    ...
    scanf("%d", &sub_choice);
    while (getchar() != '\n');
    switch (sub_choice) {               /* 将数字映射为字符串 */
        case 1: strcpy(a.subject, "科目一"); break;
        case 2: strcpy(a.subject, "科目二"); break;
        case 3: strcpy(a.subject, "科目三"); break;
        case 4: strcpy(a.subject, "科目四"); break;
        default:
            printf("[错误] 无效科目！\n");
            pause_screen();
            return;
    ...
    while (getchar() != '\n');
    if (!is_date_valid(a.exam_date)) {  /* 调用 common.c 中的日期校验 */
        printf("[错误] 日期格式不正确！\n");
        pause_screen();
        return;
    }
    ...
    while (getchar() != '\n');
    switch (slot_choice) {
        case 1: strcpy(a.exam_time, "08:30-10:30"); break;
        case 2: strcpy(a.exam_time, "13:00-15:00"); break;
        case 3: strcpy(a.exam_time, "15:30-17:30"); break;
        default:
            printf("[错误] 无效时段！\n");
            pause_screen();
            return;
    ...
    for (int i = 0; i < venue_count; i++) {
        if (venues[i].id == a.venue_id) { venue_idx = i; break; }
    }
    if (venue_idx == -1) {              /* 未找到该场地 */
        printf("[错误] 场地ID不存在！\n");
        pause_screen();
        return;
    }
    ...
    for (int i = 0; i < appointment_count; i++) {
        if (strcmp(appointments[i].exam_date, a.exam_date) == 0 &&   /* 日期相同 */
            ...
            appointments[i].venue_id == a.venue_id) {                /* 场地相同 */
            current_count++;
        }
    }
...
void list_appointments(void) {
    ...
    for (int i = 0; i < appointment_count; i++) {
        /* 权限过滤: 考生只能看自己的预约 */
        if (current_role == 2 && appointments[i].student_id != current_user_id) continue;
...
void query_appointment(void) {
    ...
    if (choice == 1) {
        ...
        if (!found) printf("该考生暂无预约记录。\n");
        else printf("共找到 %d 条预约。\n", found);
    }
    pause_screen();
}
...
void cancel_appointment(void) {
    ...
    int idx = -1;
    for (int i = 0; i < appointment_count; i++) {
        if (appointments[i].id == id) { idx = i; break; }
    }
    if (idx == -1) {                    /* 未找到 */
        printf("[错误] 未找到该预约单号！\n");
        pause_screen();
...
void update_appointment(void) {
    ...
    if (choice == 1) {
        ...
        while (getchar() != '\n');
        switch (s) {
            case 1: strcpy(appointments[idx].exam_time, "08:30-10:30"); break;
            case 2: strcpy(appointments[idx].exam_time, "13:00-15:00"); break;
            case 3: strcpy(appointments[idx].exam_time, "15:30-17:30"); break;
            default:
                printf("[错误] 无效时段！\n");
                pause_screen();
                return;
        ...
        int found = 0;
        for (int i = 0; i < venue_count; i++) {
            if (venues[i].id == vid) { found = 1; break; }
        }
        if (!found) {
            printf("[错误] 场地ID不存在！\n");
            pause_screen();
    ...
    int current_count = 0;
    for (int i = 0; i < appointment_count; i++) {
        if (i == idx) continue;          /* 跳过当前正在被修改的记录 */
